...
- **`tuist build` takes the configuration from its OWN `--configuration` flag, never from a passthrough one.** Anything after `--` goes to xcodebuild, but Tuist copies the products from the configuration *it* knows about, so `tuist build MainApp -- -configuration Release` builds one thing and files it under `Debug-iphonesimulator` — with a green exit code. It ignores a passthrough `-sdk iphoneos` the same way and produces a simulator build. This silently broke two tasks until 2026-08-07 (`prod` could not find its own output; `size-report` measured a debug simulator build against the device limit). Put `--configuration` before `--`; when you need a device SDK, go through `tuist xcodebuild build`, where every xcodebuild flag is honoured. Verify by reading the "Copying build products to …" line, not the exit code.
