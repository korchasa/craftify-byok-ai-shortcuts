...
- `./run ci` — the same gate up to the comment scan, without the simulator install and log stream, so it **exits on its own** with a meaningful status. Prefer it when you only need a verdict; this is also what GitHub Actions runs on every pull request and on `main`.
